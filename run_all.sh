...
run_test() {
    ...
    if [ -s "$perf_file" ]; then
        mem_kb=0
        last_line=$(tail -n 1 "$perf_file")
        runtime=$(echo "$last_line" | awk '{print $1}')
        mem_kb=$(echo "$last_line" | awk '{print $2}')
        if [[ "$mem_kb" =~ ^[0-9]+$ ]]; then
            mem_mb="$(awk "BEGIN {printf \"%.2fMB\", $mem_kb/1024}")"
        fi
    fi
    rm -f "$perf_file"
    ...
    if [ $exit_code -eq 124 ]; then
        ...
        verdict="CRASH"
        ((crashed++))
    elif awk "BEGIN {exit !($runtime > $time_limit)}"; then
        verdict="TIMEOUT"
        runtime=$time_limit
        ((failed++))
